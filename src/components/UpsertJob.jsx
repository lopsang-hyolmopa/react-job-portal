...
import { editJob, addJob } from "../api";

const UpsertJob = ({ isEdit }) => {
  const job = useLoaderData();

  const [title, setTitle] = useState(isEdit ? job.title : "");
  const [experience, setExperience] = useState(
    isEdit ? job.experience : "No Experience Required"
  );
  const [type, setType] = useState(isEdit ? job.type : "Full-Time");
  const [salary, setSalary] = useState(isEdit ? job.salary : "Negotiable");
  const [workplace, setWorkplace] = useState(
    isEdit ? job.workplace : "On-site"
  );
  const [description, setDescription] = useState(isEdit ? job.description : "");
  const [companyName, setCompanyName] = useState(
    isEdit ? job.company.name : ""
  );
  const [location, setLocation] = useState(isEdit ? job.company.location : "");
  const [contactEmail, setContactEmail] = useState(
    isEdit ? job.company.contactEmail : ""
  );
  const [contactPhone, setContactPhone] = useState(
    isEdit ? job.company.contactPhone : ""
  );
  const [companyDescription, setCompanyDescription] = useState(
    isEdit ? job.company.description : ""
  );
  ...
  const submitHandler = (e) => {
    ...
    const job = {
      id: isEdit ? id : undefined, // Set id only if editing
      title,
      experience,
      type,
      salary,
    ...
    const action = isEdit ? editJob : addJob;
    const successMessage = isEdit
      ? "Job listing edited successfully"
      : "Job listing added successfully";
    const destination = isEdit ? `/jobs/${id}` : "/jobs";
  ...
  return (
    <section className="bg-gray-50">
      <div className="container m-auto py-6 px-6 flex items-center gap-1.5">
        <Link to="/">Home</Link>
        <AiOutlineRight />
        <span className="text-blue-700">{isEdit ? "Edit Job" : "Add Job"}</span>
      </div>
